...
struct BassSplashGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://bassplash.top/check")! }
    
    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            BassSplashEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    BassSplashGameInitialView()
}
